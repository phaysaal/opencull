...
class ProviderStore:
    ...
    def materialize_custom(
        ...
        """A bundle for one of the photographer's own programs.

        The caller (the program store) has already resolved the `use`
        lines to absolute paths; this generates the same agents.kim the
        built-ins get -- key_env, never the key -- rewrites the agents
        use to it, and files the bundle with the program's hash pinned,
        exactly like a shipped program's run.
        """
        source_path = Path(source_path).expanduser().resolve()
        with self._lock:
            profile = deepcopy(self._profile(profile_id))
        env_name = f"OPENCULL_PROVIDER_TOKEN_{profile_id.upper()}"
        agents_text = self._agents_text(profile, profile_id, env_name)
        bundle = (self.generated_root / job_id).resolve()
        agents_path = bundle / "agents.kim"
        program_path = bundle / source_path.name
        source = re.sub(
            r'use\s+"[^"]*agents\.kim"',
            f'use "{agents_path}"', resolved_source, count=1)
        ...
        manifest = {
            ...
            "job_id": job_id,
            "profile": profile,
            "agents_sha256": hashlib.sha256(agents_text.encode()).hexdigest(),
            "program_sha256": hashlib.sha256(source.encode()).hexdigest(),
            "agents_path": str(agents_path),
            "program_path": str(program_path),
            "program_name": source_path.name,
            "program_source": str(source_path),
    ...
    def _agents_text(self, profile: dict[str, Any], profile_id: str,
                     env_name: str) -> str:
        lines = [
